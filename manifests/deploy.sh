#!/usr/bin/env bash
# deploy.sh
# Deploys Iskoces to a Kubernetes cluster
...
echo ""
log_info "To use with Glooscap, configure:"
echo "  TRANSLATION_SERVICE_ADDR=${SERVICE_ADDR}"
echo "  TRANSLATION_SERVICE_TYPE=iskoces"
echo "  TRANSLATION_SERVICE_SECURE=false"
echo ""
log_info "To view logs:"
echo "  kubectl logs -f -n iskoces deployment/iskoces-server"
echo ""
log_info "To test gRPC connection:"
echo "  kubectl run -it --rm debug --image=curlimages/curl --restart=Never -- grpc_health_probe -addr=${SERVICE_ADDR}"
